...
show_mainmenu = False
in_server = False
sfw_running_first_time = True
sdsmw_running_first_time = True
...
def show_dsm_window(transition, custom_tab):
    global sdsmw_running_first_time
    if sdsmw_running_first_time:
        DetailSettingsManagerWindow(transition, custom_tab)
        sdsmw_running_first_time = False
    else: DetailSettingsManagerWindow(transition, custom_tab)

class DetailSettingsManagerWindow(bauiv1.Window):
    def __init__(self, transition, custom_tab):
        global current_tab, sdsmw_running_first_time, dsmw_root_widget
        ...
        if not sdsmw_running_first_time:
            bauiv1.containerwidget(edit=dsmw_root_widget, transition='out_scale')
        ...
        dsmw_root_widget = self._root_widget
...
class NewMainMenu:
    global quick_language, show_gather_window, confirm_reconnect_server, reconnect_server, show_dsm_window
    ...
    def _new_refresh(self):
        ...
        if not self._in_game:
            self._detail_settings_manager_button = bauiv1.buttonwidget(
                ...
                size=(81, 76+extra),
                label='',
                on_activate_call=babase.Call(show_dsm_window, 'in_scale', ''))
    ...
    def confirm_reconnect_server():
        if bascenev1.get_connection_to_host_info_2():
...
class NewPartyWindow:
    ...
    def _new_init(self, origin: Sequence[float] = (0, 0)):
        ...
        info = bascenev1.get_connection_to_host_info_2()
        if info != None:
    ...
    def _new_send_chat_message(self):
        ...
        if msg == config['Ping Command']:
            ...
            show_dsm_window(transition='in_scale', custom_tab='pw_tab')
            bauiv1.containerwidget(edit=self._root_widget, transition='out_scale')
            bauiv1.textwidget(edit=self._text_field, text='')
        elif msg == '!dsw':
            show_dsm_window(transition='in_scale', custom_tab='')
            bauiv1.containerwidget(edit=self._root_widget, transition='out_scale')
            bauiv1.textwidget(edit=self._text_field, text='')
    ...
    def _new_popup_menu_selected_choice(self, popup_window: PopupMenuWindow, choice: str) -> None:
                ...
                if choice == 'add to favorites':
                    ...
                    if info is not None:
                        ...
                        bauiv1.screenmessage(
                            bauiv1.Lstr(resource='errorText'), color=(1, 0, 0)
                        )
                        bauiv1.getsound('error').play()
            else:
    ...
    def _new_on_party_member_press(self, client_id: int, is_host: bool,
        ...
        if bascenev1.get_foreground_host_session() is not None:
            ...
            if bascenev1.get_connection_to_host_info_2():
                if (bascenev1.get_connection_to_host_info_2().build_number <
                        14248):
                    return
    ...
    def _on_menu_button_press(self) -> None:
        ...
        server_info = bascenev1.get_connection_to_host_info_2()
    ...
    def _new_update(self, update_type='auto'):
        ...
        if roster != self._roster or update_type == 'manual':
            self._roster = roster
                    ...
                    for x in range(columns):
                        ...
                        if index < len(self._roster):
                            ...
                            try:
                                ...
                                p_str = (self._roster[index]['display_string'] if player_type == 'Display String' else
                                         str(self._roster[index]['client_id']) if player_type == 'Client ID' else p_str)
                            except Exception:
                                logging.exception(
...
class NewOnScreenKeyboardWindow:
    ...
    def _new_init(self, adapter: StringEditAdapter):
        ...
        self._shift_button: bauiv1.Widget | None = None
        self._backspace_button: bauiv1.Widget | None = None
        self._space_button: bauiv1.Widget | None = None
        self._double_press_shift = False
        self._num_mode_button: bauiv1.Widget | None = None
        self._emoji_button: bauiv1.Widget | None = None
        self._char_keys: list[bauiv1.Widget] = []
        self._keyboard_index = 0
        self._last_space_press = 0.0
        self._double_space_interval = 0.3

        self._keyboard: bauiv1.Keyboard
...
def _new_disconnect_from_host():
    ...
    if config['Save History']:
        ...
        _history = {'entry_time': entry_time,
                    ...
                    'server_name': bascenev1.get_connection_to_host_info_2().name,
                    'server_ip': server_ip,
...
def run_ping_thread():
    if bascenev1.get_connection_to_host_info_2(): PingThread(server_ip, server_port).start()
    else:
        babase.screenmessage('You are not on a server', color=(1,0,0))
        bauiv1.getsound('error').play()
